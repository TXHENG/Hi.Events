...
import {DrawerStat, SideDrawer, SideDrawerFields, SideDrawerHeading, SideDrawerSection, SideDrawerStats} from '../../common/SideDrawer';
...
export const ManageOrderModal = ({onClose, orderId}: GenericModalProps & ManageOrderModalProps) => {
    ...
    const occurrences = Array.from(new Map((order.order_items ?? []).filter(item => item.event_occurrence).map(item => [item.event_occurrence!.id, item.event_occurrence!])).values());
            ...
            ? [
                  {
                      ...
                      value: occurrences.map(occurrence => (
                          <div key={occurrence.id}>
                              {formatDateWithLocale(occurrence.start_date, 'shortDate', event.timezone)} {formatDateWithLocale(occurrence.start_date, 'timeOnly', event.timezone)}
                    ...
                    <CopyButton value={String(order.public_id)}>
                        {({copied, copy}) => (
                            <Tooltip label={copied ? t`Copied` : t`Copy`} position="bottom" withArrow>
                                <ActionIcon variant="subtle" color="gray" size="sm" radius="xl" aria-label={t`Copy`} onClick={copy}>
    ...
    return (
        <SideDrawer opened={true} onClose={onClose} header={header} footer={footer}>
            {isEditing ? (
                <form id={FORM_ID} onSubmit={form.onSubmit(handleSubmit)} className={classes.form}>
                    ...
                    <TextInput {...form.getInputProps('email')} label={t`Email address`} placeholder="homer@simpson.com" required />
                    <Textarea label={<InputLabelWithHelp label={t`Notes`} helpText={t`Add any notes about the order. These will not be visible to the customer.`} />} {...form.getInputProps('notes')} placeholder={t`Add any notes about the order...`} minRows={3} maxRows={6} autosize />
                </form>
            ) : (
                <>
                    ...
                    {(order.status === 'AWAITING_OFFLINE_PAYMENT' || order.payment_proof_status !== undefined) && (
                    ...
                    {attendees.length > 0 && (
                        <SideDrawerSection title={t`Attendees`} count={attendees.length}>
                            <AttendeeList refetchOrder={refetchOrder} order={order} products={products as Product[]} questionAnswers={questionAnswers} />
                        </SideDrawerSection>
                    )}

                    {questionAnswers.length > 0 && (
                        <SideDrawerSection title={t`Questions & Answers`} count={questionAnswers.length}>
                            <QuestionAndAnswerList onEditAnswer={refetchOrder} questionAnswers={questionAnswers as QuestionAnswer[]} />
                        </SideDrawerSection>
                    )}
                </>
            )}
