        ...
        <div style={{marginTop: '20px', marginBottom: '40px'}}>
            ...
            <Card>
                <Stack gap="md">
                    ...
                    {proofs.map(proof => (
                        <Alert key={proof.id} icon={proof.status === 'APPROVED' ? <IconCheck size={16} /> : <IconInfoCircle size={16} />} color={proof.status === 'REJECTED' ? 'red' : proof.status === 'APPROVED' ? 'green' : 'blue'}>
                    ...
                    {!hasPendingProof && (
                        <>
                            <FileInput label={t`Payment receipt`} description={t`JPG, PNG, or PDF up to 10 MB`} placeholder={t`Choose a file`} accept="image/jpeg,image/png,application/pdf" value={file} onChange={setFile} clearable />
                            <TextInput label={t`Payment reference`} description={t`Optional bank transfer or transaction reference`} value={paymentReference} onChange={event => setPaymentReference(event.currentTarget.value)} />
                            <Button leftSection={<IconFileUpload size={16} />} disabled={!file} loading={submitMutation.isPending} onClick={() => submitMutation.mutate()}>
                                {t`Submit proof`}
                            </Button>
                        </>
                    )}
