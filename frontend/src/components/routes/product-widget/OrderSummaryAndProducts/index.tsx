...
import {IconBuilding, IconCalendar, IconCalendarEvent, IconCash, IconCheck, IconClock, IconEdit, IconExternalLink, IconId, IconMail, IconMapPin, IconMenuOrder, IconPrinter, IconSend, IconTicket, IconUser} from '@tabler/icons-react';
...
const GuestListItem = ({attendee, event, allowSelfEdit, onEditClick, onResendClick}: {attendee: Attendee; event: Event; allowSelfEdit: boolean; onEditClick: () => void; onResendClick: () => void}) => {
        ...
        <div className={`${classes.guestItem} ${isCancelled ? classes.guestItemCancelled : ''}`}>
            ...
            <div className={classes.guestActions}>
                <Tooltip label={t`View Ticket`}>
                    <ActionIcon variant="subtle" onClick={() => window?.open(`/product/${event.id}/${attendee.short_id}`, '_blank')}>
                        <IconExternalLink size={18} />
                    </ActionIcon>
                </Tooltip>
                <Tooltip label={t`Print Ticket`}>
                    <ActionIcon variant="subtle" onClick={() => window?.open(`/product/${event.id}/${attendee.short_id}/print`, '_blank')}>
                        <IconPrinter size={18} />
                    </ActionIcon>
                </Tooltip>
                {allowSelfEdit && !isCancelled && (
                    <>
                        <Tooltip label={t`Edit Attendee`}>
                            <ActionIcon variant="subtle" data-testid="attendee-edit-button" onClick={onEditClick}>
                                <IconEdit size={18} />
                            </ActionIcon>
                        </Tooltip>
                        <Tooltip label={t`Resend Ticket`}>
                            <ActionIcon variant="subtle" data-testid="resend-ticket-button" onClick={onResendClick}>
                                <IconSend size={18} />
                            </ActionIcon>
                        </Tooltip>
                    </>
    ...
    <div className={classes.detailItem}>
        <Group gap="xs" wrap="nowrap">
            <Icon
                size={20}
                style={{
                    color: 'var(--checkout-accent, var(--mantine-color-gray-6))',
                    flexShrink: 0
                }}
            />
...
const WelcomeHeader = ({order, event, allowSelfEdit, hasSubmittedPaymentProof}: {order: Order; event: Event; allowSelfEdit: boolean; hasSubmittedPaymentProof: boolean}) => {
    ...
    const message = {
        ...
        RESERVED: null,
        AWAITING_OFFLINE_PAYMENT: hasSubmittedPaymentProof ? t`Payment proof submitted` : t`Your order is awaiting payment`,
        ABANDONED: null
    }[order.status];
    ...
    return (
        <div className={classes.welcomeHeader}>
            ...
            {isCompleted && allowSelfEdit && <div className={classes.selfServiceHint}>{t`Bookmark this page to manage your order anytime.`}</div>}
            {isAwaitingPayment && hasSubmittedPaymentProof && <div className={classes.confirmationText}>{t`We received your payment proof. You can close this page; the organizer will contact you if anything else is needed.`}</div>}
            {isCancelled && (
                <div className={classes.confirmationText}>
...
const OrderDetails = ({order, event, allowSelfEdit, onEditClick, onResendClick}: {order: Order; event: Event; allowSelfEdit: boolean; onEditClick: () => void; onResendClick: () => void}) => (
    <Card style={{marginBottom: '40px'}}>
        <SimpleGrid cols={{base: 1, sm: 2}} spacing="md">
            ...
            />
            <DetailItem icon={IconId} label={t`Order Reference`} value={order.public_id} />
            <DetailItem
                icon={IconMail}
                label={t`Email`}
                value={
            ...
            />
            <DetailItem icon={IconCalendar} label={t`Order Date`} value={dateToBrowserTz(order.created_at, event.timezone)} />
            {!!order.refund_status && <DetailItem icon={IconMenuOrder} label={t`Refund Status`} value={<RefundStatusType order={order} />} />}
            {order.payment_status !== 'PAYMENT_RECEIVED' && order.payment_status !== 'NO_PAYMENT_REQUIRED' && <DetailItem icon={IconCash} label={t`Payment Status`} value={<PaymentStatus order={order} />} />}
            {order.address && <DetailItem icon={IconMapPin} label={t`Billing Address`} value={formatAddress(order.address)} />}
        </SimpleGrid>
    </Card>
);
...
const EventDetails = ({event, order}: {event: Event; order: Order}) => {
    ...
    const venueName = isInPerson ? effective.location?.name || effective.location?.structured_address?.venue_name || null : null;
    const formattedAddress = isInPerson && effective.location?.structured_address ? formatAddress(effective.location.structured_address) : '';
    const venueDetails = isInPerson ? (venueName ? `${venueName}${formattedAddress ? `, ${formattedAddress}` : ''}` : formattedAddress || null) : null;
            ...
            <SimpleGrid cols={{base: 1, sm: 2}} spacing="md">
                ...
                {venueDetails && (
                    <DetailItem
                        ...
                        label={t`Location`}
                        value={
                            <NavLink to={mapsUrl} target="_blank">
                                {venueDetails}
                            </NavLink>
                        }
                    />
                )}
                <DetailItem icon={IconClock} label={t`Timezone`} value={event.timezone} />
                <DetailItem
                    icon={IconBuilding}
                    label={t`Organizer`}
                    value={
                        <>
                            {event.organizer?.email && <NavLink to={event.organizer?.email ? `mailto:${event.organizer.email}` : '#'}>{event.organizer?.name}</NavLink>}
...
const OrderStatus = ({order}: {order: Order}) => {
    if (order?.status === 'CANCELLED') {
        return <HomepageInfoMessage status="cancelled" message={t`Order cancelled`} subtitle={t`This order has been cancelled.`} />;
    }

    if (order?.status === 'COMPLETED') {
        return <HomepageInfoMessage status="success" message={t`Order complete`} subtitle={t`This order is complete.`} />;
    }

    return <HomepageInfoMessage status="processing" message={t`Processing order`} subtitle={t`This order is being processed.`} />;
};
...
export const OrderSummaryAndProducts = () => {
    ...
    if (isError) {
        return <HomepageInfoMessage status="not_found" message={t`Order Not Found`} subtitle={t`We couldn't find the order you're looking for. The link may have expired or the order details may have changed.`} />;
    }
    ...
    const hasSubmittedPaymentProof = order.status === 'AWAITING_OFFLINE_PAYMENT' && order.payment_proof_status === 'PENDING';
    ...
    return (
        <>
            <CheckoutContent>
                ...
                <WelcomeHeader order={order} event={event} allowSelfEdit={allowSelfEdit} hasSubmittedPaymentProof={hasSubmittedPaymentProof} />
                ...
                <InlineOrderSummary event={event} order={order} showBuyerProtection={false} defaultExpanded={false} />

                {order?.status === 'AWAITING_OFFLINE_PAYMENT' && !hasSubmittedPaymentProof && <OfflinePaymentInstructions event={event} />}
                {order?.status === 'AWAITING_OFFLINE_PAYMENT' && !hasSubmittedPaymentProof && event?.settings?.allow_offline_payment_proof && <OfflinePaymentProofUpload event={event} orderShortId={order.short_id} />}

                <h1 className={classes.heading}>{t`Order Details`}</h1>

                <OrderDetails order={order} event={event} allowSelfEdit={allowSelfEdit} onEditClick={() => setEditOrderModalOpened(true)} onResendClick={handleResendOrderConfirmation} />
                ...
                {order?.attendees && order.attendees.length > 0 && (
                    <>
                        <Group justify="space-between" align="center">
                            ...
                            <Button size="sm" variant="subtle" leftSection={<IconPrinter size={16} />} onClick={() => window?.open(`/order/${eventId}/${orderShortId}/print`, '_blank')}>
                                {t`Print All Tickets`}
                            </Button>
                        </Group>
                        ...
                        <Card>
                            <div className={classes.guestList}>
                                {order.attendees.map(attendee => (
                                    <GuestListItem key={attendee.id} attendee={attendee} event={event} allowSelfEdit={allowSelfEdit} onEditClick={() => setEditingAttendee(attendee)} onResendClick={() => handleResendAttendeeTicket(attendee)} />
                                ))}
                            </div>
                        </Card>
                    </>
